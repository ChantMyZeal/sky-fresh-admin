<template>
  <div class="navbar">
    <div class="statusBox">
      <span v-if="status===1"
            class="businessBtn">正在营业</span>
      <span v-else
            class="businessBtn closing">暂停营业</span>
    ...
    <div :key="restKey"
      ...
      <div class="rightStatus">
        ...
        <span class="navicon operatingState" @click="handleStatus" v-if="user === 'admin'"><i />店铺状态设置</span>
      </div>
      <!-- 店铺信息设置按键 -->
      <div class="rightStatus" v-if="user === 'admin'">
        <span class="navicon operatingInfo" @click="handleInfo"><i />店铺信息设置</span>
      </div>
      <!-- 身份操作按键 -->
    ...
    <el-dialog title="店铺状态设置"
      ...
      <el-radio-group v-model="setStatus">
        <el-radio :label="1">
          恢复营业
          <span>店铺正在营业，自动接收订单信息，可点击暂停营业进入暂停营业状态。</span>
        </el-radio>
        <el-radio :label="0">
          暂停营业
          <span>店铺暂停营业，仅处理进行中的订单，可点击恢复营业进入正在营业状态。</span>
        </el-radio>
      </el-radio-group>
      <span slot="footer"
            class="dialog-footer">
    ...
    <!-- end -->
    <!-- 修改密码弹层 -->
    <Password :dialog-pwd-visible="dialogPwdVisible"
              @handleclose="handlePwdClose" @logout="logout" />
    <!-- end -->

    <!-- 店铺信息弹层 -->
    <Information :dialog-info-visible="dialogInfoVisible"
...
export default class extends Vue {
  ...
  private status = 1
  private setStatus = 1
  private dialogPwdVisible = false
  private dialogInfoVisible = false
  ...
  webSocket() {
    ...
    let socketUrl = 'ws://' + location.host + process.env.VUE_APP_SOCKET_URL + clientId
