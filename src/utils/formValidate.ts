...
// 获取近几日（自定义日期）
export const pastDays = (days:number) => {
  let toData = new Date(new Date().toLocaleDateString()).getTime();
  let pastDaysStart = toData - days * 3600 * 24 * 1000;
  let pastDaysEnd = toData - 1;
  let daysStart = dateFormat("YYYY-mm-dd", pastDaysStart);
  let daysEnd = dateFormat("YYYY-mm-dd", pastDaysEnd);
  return [daysStart, daysEnd];
};

// 自定义日期范围的格式化
export const rangeDays = (range:[string, string]) => {
  let daysStart = dateFormat("YYYY-mm-dd", range[0]);
  let daysEnd = dateFormat("YYYY-mm-dd", range[1]);
  return [daysStart, daysEnd];
};
