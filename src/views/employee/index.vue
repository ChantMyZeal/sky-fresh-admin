        ...
        <el-table-column label="操作" width="200" align="center">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              size="small"
              class="blueBug"
              :class="{'disabled-text': user !== 'admin' || scope.row.username === 'admin'}"
              :disabled="user !== 'admin' || scope.row.username === 'admin'"
              @click="addEmployeeHandle(scope.row.id, scope.row.username)"
            >
              修改
            </el-button>
            ...
            <el-button
              ...
              class="non"
              :class="{
                'disabled-text': user !== 'admin' || scope.row.username === 'admin',
                blueBug: scope.row.status == '0',
                delBut: scope.row.status != '0'
              }"
              @click="statusHandle(scope.row)"
...
export default class extends Vue {
  ...
  private deleteHandle(row: any) {
    if (
      // row.username === 'admin'
      this.user !== 'admin' || row.username === 'admin'
      ) {
      return
    }
    this.id = row.id
